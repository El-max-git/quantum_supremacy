 ...
 * с поддержкой специальных блоков и ссылок
 */
...
class ArticleParser {
    ...
    async parse(markdownText, articlePath = '') {
        ...
        try {
            ...
            // Парсим markdown без обработки формул
            ...
            // 3. Post-process: изображения
            html = this.processImages(html, articlePath);
            
            // 4. Post-process: внутренние ссылки
            html = this.processInternalLinks(html, articlePath);
            
            // 5. Post-process: автоматические якоря
...
${cleanContent}
    ...
    async convertMarkdownToHtml(markdownText) {
        ...
        // Формулы ($...$ и $$...$$) не обрабатываются - остаются как есть
        marked.setOptions({
            breaks: true,
            gfm: true,
            headerIds: true,
        ...
        // Кастомный рендерер для кода
        // ВАЖНО: Блоки кода НЕ преобразуются в формулы - остаются как есть
        renderer.code = (code, language) => {
            // Экранируем код, чтобы предотвратить любую обработку формул
            // escapeHtml экранирует HTML спецсимволы, но НЕ экранирует $, поэтому $ остаются как есть в исходном виде
            const escapedCode = this.escapeHtml(code);
            if (!language) {
                // ASCII-диаграммы без подсветки
                return `<pre class="ascii-diagram"><code>${escapedCode}</code></pre>`;
            }
        ...
        // ДОПОЛНИТЕЛЬНАЯ ЗАЩИТА: Убеждаемся, что блоки кода не содержат добавленных $ к формулам
        // Это защита от того, что marked.js или другие расширения могли добавить $ к формулам в блоках кода
        // Проверяем, что в блоках кода нет формул, обернутых в $, если их не было в исходном коде
        // Но мы не можем проверить исходный код здесь, поэтому оставляем как есть
